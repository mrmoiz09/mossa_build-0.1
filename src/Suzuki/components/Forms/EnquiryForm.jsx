...
const navigate = useNavigate();
    const { handleSubmit, register, formState: { errors } ,reset} = useForm({
      ...
      const [defaultCarValue,setDefaultCarValue] = useState("");
      ...
      const onSubmit = async (data) => {
        console.log(data);
        setLoading(true);
        await callApi(HomeApi.test_drive.method,HomeApi.test_drive.url,null,data,defaultHeader())
        .then((res)=>{
          if(!res?.data?.error)
          {
            setLoading(false);
            ...
            navigate(`/${SuzukiRoutes.thankyou}/enquiry`);
          }
          else
          {
            setLoading(false);
      ...
      const fetchMstCar = useCallback(async () => {
        await callApi(CommonApi.getCarsOfSuzuki.method, CommonApi.getCarsOfSuzuki.url, null, null, defaultHeader()).then((res) => {
          if (!res?.data?.error) {
            const payload = res?.data?.data;
            if (Array.isArray(payload)) {
              setCarList(payload);
              setDefaultCarValue(typeof(params?.id)=="undefined" ? 4 : params?.id);
            }
          } 
        }).catch((err) => {
          console.log(err);
            ...
            <div className="col">
                <div className="form-group">
                    ...
                    >
                        ...
                        {carList.length>0 ? 
                          carList.map((v,i)=>(
                            <option value={v?.id} selected={v?.id==defaultCarValue}>{v?.title}</option>
                          ))
                        :""}
                    </select>
